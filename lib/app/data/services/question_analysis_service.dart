...
//   ///
//   /// DI = (عدد الإجابات الصحيحة) / (إجمالي المحاولات)
//   ///
//   /// التفسير:
//   /// - DI > 0.9: سؤال سهل جداً (معظم الطلاب يجيبون صحيح)
//   /// - 0.7 < DI < 0.9: سؤال سهل
//   /// - 0.3 < DI < 0.7: سؤال متوسط (مثالي)
//   /// - 0.1 < DI < 0.3: سؤال صعب
//   /// - DI < 0.1: سؤال صعب جداً (يحتاج مراجعة)
...
//   ///
//   /// يقيس قدرة السؤال على التفريق بين الطلاب الأقوياء والضعفاء
//   ///
//   /// الخطوات:
//   /// 1. ترتيب الطلاب حسب الدرجة الإجمالية
//   /// 2. أخذ أعلى 27% (المجموعة القوية)
//   /// 3. أخذ أقل 27% (المجموعة الضعيفة)
//   /// 4. DiscI = (نسبة الأقوياء الذين أجابوا صح) - (نسبة الضعاف الذين أجابوا صح)
//   ///
//   /// التفسير:
//   /// - DiscI > 0.4: سؤال ممتاز (يميز جيداً)
//   /// - 0.3 < DiscI < 0.4: سؤال جيد
//   /// - 0.2 < DiscI < 0.3: سؤال مقبول
//   /// - DiscI < 0.2: سؤال ضعيف (لا يميز - يحتاج مراجعة)
//   /// - DiscI < 0: سؤال معكوس (الضعفاء يجيبون أفضل من الأقوياء! خطأ في السؤال)
...
//     // أخذ أعلى وأقل 27%
//     final topCount = (sorted.length * 0.27).ceil();
//     if (topCount < 1) return 0.3; // عدد قليل جداً من البيانات

//     final topStudents = sorted.take(topCount).toList();
//     final bottomStudents = sorted.skip(sorted.length - topCount).toList();

//     // حساب نسبة الإجابات الصحيحة لكل مجموعة
...
//     final topRate = topCorrect / topCount;
//     final bottomRate = bottomCorrect / topCount;
...
//   /// تحليل مجموعة من الأسئلة دفعة واحدة
//   Future<List<QuestionModel>> analyzeBatch({
//     required List<QuestionModel> questions,
//     required List<StudentAttempt> attempts,
//   }) async {
//     final analyzed = <QuestionModel>[];

//     for (final question in questions) {
//       final updated = await analyzeAndUpdateQuestion(
//         question: question,
//         attempts: attempts,
//       );
//       analyzed.add(updated);
//     }
